...
Секреты Directus и PostgreSQL не входят в image, Git или build args. Независимые read-only `DIRECTUS_PUBLIC_TOKEN` и `DIRECTUS_PREVIEW_TOKEN` передаются в `DIRECTUS_STATIC_TOKEN` только соответствующего server runtime; admin token frontend-контейнеры не получают. Public policy читает только физические `published_*` projections и не имеет доступа к исходным content-коллекциям или `language_versions`; preview policy читает исходные коллекции и версии. Проекции обновляются PostgreSQL trigger-функцией в той же транзакции, что и изменение исходных строк.
