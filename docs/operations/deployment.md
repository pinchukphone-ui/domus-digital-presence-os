...
Секреты Directus и PostgreSQL не входят в image, Git или build args. Read-only `DIRECTUS_FRONTEND_TOKEN` передаётся в `DIRECTUS_STATIC_TOKEN` только server runtime; admin token frontend-контейнеры не получают. Перед внешним deployment создайте разные public/preview service tokens.
