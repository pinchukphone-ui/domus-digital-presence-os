...
export function MortgageCalculator({ language }: { language: 'pl' | 'ru' }) {
  ...
  const labels = language === 'pl'
    ? { amount: 'Kwota kredytu (PLN)', years: 'Okres (lata)', rate: 'Oprocentowanie (%)', result: 'Szacowana rata miesięczna', suffix: 'PLN / mies.', note: 'Wynik orientacyjny. To nie jest oferta ani rekomendacja finansowa.' }
    : { amount: 'Сумма кредита (PLN)', years: 'Срок (лет)', rate: 'Ставка (%)', result: 'Ориентировочный платёж в месяц', suffix: 'PLN / мес.', note: 'Расчёт ориентировочный. Это не предложение и не финансовая рекомендация.' };
