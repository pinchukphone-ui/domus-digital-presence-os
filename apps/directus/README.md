...
Directus 12.1.1 запускается из image, закреплённого одновременно по версии и multi-platform digest. Пользовательские коллекции создаются SQL migration при первом старте PostgreSQL и автоматически отражаются Directus. Admin token используется только bootstrap/операциями, а Astro получает отдельный `DIRECTUS_FRONTEND_TOKEN` с пятью read-only permissions; `language_versions` читается только серверным preview-контуром. Не передавайте токены в браузер.
