...
public_pages="$(request_status -H "Authorization: Bearer $DIRECTUS_PUBLIC_TOKEN" 'http://127.0.0.1:8055/items/pages?limit=1')"
preview_pages="$(request_status -H "Authorization: Bearer $DIRECTUS_PREVIEW_TOKEN" 'http://127.0.0.1:8055/items/pages?limit=1')"
public_write="$(request_status -X POST -H "Authorization: Bearer $DIRECTUS_PUBLIC_TOKEN" -H 'Content-Type: application/json' --data '{}' 'http://127.0.0.1:8055/items/pages')"
...
if [[ "$public_versions" != 403 || "$preview_versions" != 200 || "$public_pages" != 200 || "$preview_pages" != 200 || "$public_write" != 403 || "$preview_write" != 403 ]]; then
  printf 'Renderer access verification failed: public_versions=%s preview_versions=%s public_pages=%s preview_pages=%s public_write=%s preview_write=%s\n' \
    "$public_versions" "$preview_versions" "$public_pages" "$preview_pages" "$public_write" "$preview_write" >&2
  exit 1
fi

printf 'Renderer access verified: pages 200/200, public versions 403, preview versions 200, writes 403/403\n'
